...
uint32_t AW2013_Init(void)
{
    ...
    // 设置RGB三路通道的工作模式 (0x01: 直接PWM控制模式)
    result = aw2013_WiteByte(LCFG0_REG_ADDR, 0x01);
    if (result != HI_ERR_SUCCESS) {
        printf("I2C aw2013 LCFG0_REG_ADDR status = 0x%x!!!\r\n", result);
        return result;
    }
    result = aw2013_WiteByte(LCFG1_REG_ADDR, 0x01);
    if (result != HI_ERR_SUCCESS) {
        printf("I2C aw2013 LCFG1_REG_ADDR status = 0x%x!!!\r\n", result);
        return result;
    }
    result = aw2013_WiteByte(LCFG2_REG_ADDR, 0x01);
    if (result != HI_ERR_SUCCESS) {
