...
extern tn_pcf8574_io_t pcf8574_io; // 使用全局统一的IO状态变量

void set_fan(uint8_t status)
{
    if (status == true) {
        pcf8574_io.bit.p0 = 1; // 风扇开启
    } else {
        pcf8574_io.bit.p0 = 0; // 风扇关闭
    }
...
{
    if (status == true) {
        pcf8574_io.bit.p5 = 1; // D2开启
    } else {
        pcf8574_io.bit.p5 = 0; // D2关闭
    }
    PCF8574_Write(pcf8574_io.all);
}
...
{
    if (status == true) {
        pcf8574_io.bit.p1 = 0; // 蜂鸣器开启 (低电平触发)
    } else {
        pcf8574_io.bit.p1 = 1; // 蜂鸣器关闭
    }
    PCF8574_Write(pcf8574_io.all);
}
...
{
    if (status == true) {
        pcf8574_io.bit.p2 = 0; // LED开启
    } else {
        pcf8574_io.bit.p2 = 1; // LED关闭
    }
    PCF8574_Write(pcf8574_io.all);
}
...
uint32_t PCF8574_Init(void)
{
    ...
    pcf8574_io.bit.p1 = 1; // 蜂鸣器关闭
    pcf8574_io.bit.p2 = 1; // LED关闭
    pcf8574_io.bit.p4 = 1; // 火焰传感器初始引脚
    pcf8574_io.bit.p5 = 0; // 可然气体传感器
    pcf8574_io.bit.p7 = 1; // LED关闭


    PCF8574_Write(pcf8574_io.all); // v4.2版本开发板 关闭风扇，关闭蜂鸣器，关闭LED灯
    return HI_ERR_SUCCESS;
}