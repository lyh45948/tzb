#include <stdio.h>
#include <unistd.h>
#include <string.h>
#include <stdlib.h>

#include "ohos_init.h"
#include "cmsis_os2.h"
#include "cJSON.h"
#include "sys_config.h"

#include "hi_io.h"
#include "hi_gpio.h"
#include "hi_uart.h"

#include "hal_bsp_ssd1306.h"
#include "hal_bsp_ssd1306_bmps.h"
#include "hal_bsp_log.h"
#include "hal_bsp_pcf8574.h"

#include "oled_show.h"

msg_data_t sys_msg_data;
hi_u8 recvBuff[200] = {0};
hi_u8 *pbuff = recvBuff;
osThreadId_t uart_recv_task_id;   // 任务 ID

margin_t   bmp_number_1   = { .top = 16+8, .left = 8, };// 数字-百位 
margin_t   bmp_number_2   = { .top = 16+8, .left = 24, };// 数字-十位 
margin_t   bmp_number_3   = { .top = 16+8, .left = 40, };// 数字-个位 
margin_t   bmp_dian       = { .top = 32+8, .left = 56 };// 小数点  
margin_t   bmp_number_4   = { .top = 32+8, .left = 72 };// 数字-小数位 
margin_t   bmp_danwei     = { .top = 16+8, .left = 88 };// 单位 

/**
 * @brief  串口初始化
 * @note   与STM32单片机之间的串口通信
 * @retval None
 */
void uart_init(void)
{
  uint32_t ret = 0;
  // 初始化串口
  hi_io_set_func(HI_IO_NAME_GPIO_11, HI_IO_FUNC_GPIO_11_UART2_TXD);
  hi_io_set_func(HI_IO_NAME_GPIO_12, HI_IO_FUNC_GPIO_12_UART2_RXD);

  hi_uart_attribute uart_param = {
      .baud_rate = 115200,
      .data_bits = 8,
      .stop_bits = 1,
      .parity = 0,
  };
  ret = hi_uart_init(HI_UART_IDX_2, &uart_param, NULL);
  if (ret != HI_ERR_SUCCESS)
  {
    console_log_error("hi uart init is faild.");
  }
}
/**
 * @brief  显示页面
 * @note   
 * @param  val: 
 * @retval None
 */
void show_page(uint16_t val)
{
  SSD1306_ShowStr(0, 0, " Smart Distance", 16);

  // 显示数字 
  uint8_t x = val / 1000; // 显示数字的千位
  SSD1306_DrawBMP(bmp_number_1.left, bmp_number_1.top, 16, 32, bmp_16X32_number[x]); 

  x = val / 100 % 10;// 显示数字的百位
  SSD1306_DrawBMP(bmp_number_2.left, bmp_number_2.top, 16, 32, bmp_16X32_number[x]); 

  x = val / 10 % 10;// 显示数字的十位
  SSD1306_DrawBMP(bmp_number_3.left, bmp_number_3.top, 16, 32, bmp_16X32_number[x]); 

  x = val % 10;// 显示数字的个位
  SSD1306_DrawBMP(bmp_number_4.left, bmp_number_4.top, 8, 16, bmp_8X16_number[x]); 
    
  // 显示小数点
  SSD1306_DrawBMP(bmp_dian.left, bmp_dian.top, 16, 16, bmp_16X16_dian); 
  // 显示cm
  SSD1306_DrawBMP(bmp_danwei.left, bmp_danwei.top, 32, 32, bmp_32X32_cm);          
}
/**
 * @brief  解析JSON数据包
 * @note   
 * @param  *pstr: 
 * @retval None
 */
void parse_json_data(void)
{
  if (strlen((char *)recvBuff) <= 3)
    return ;
  
  cJSON *json_root = cJSON_Parse((const char *)recvBuff);
  if(json_root){
    console_log_success("json_root");
    cJSON *json_status = cJSON_GetObjectItem(json_root, "status");
    if(json_status){
      console_log_success("json_status");
      cJSON *json_distance = cJSON_GetObjectItem(json_status, "distance");
      if(json_distance){
        sys_msg_data.distance = json_distance->valueint;
        show_page(sys_msg_data.distance);   // 显示页面
      } 
      json_distance = NULL;
    }
    json_status = NULL;
    cJSON_Delete(json_root);
  }
  json_root = NULL;
}

void uart_recv_task(void)
{
  hi_u8 uart_buff[20] = {0};
  hi_u8 last_len = 0;
  uart_init();    // 串口2初始化
  while (1)
  {
    // 阻塞接收
    hi_u32 len = hi_uart_read(HI_UART_IDX_2, uart_buff, sizeof(uart_buff));
    if (len > 0)
    {
      // printf("uart_buff: %s\n", uart_buff);
      memcpy((char *)pbuff, (char *)uart_buff, len);
      pbuff += len;
      if(len < last_len){
        pbuff = recvBuff;   // 回到recvBuff的头位置
        // printf("buff: %s\n", recvBuff);
        parse_json_data();
        memset((char *)recvBuff, 0, sizeof(recvBuff));
      }
      last_len = len;
      memset((char *)uart_buff, 0, sizeof(uart_buff));
    }
  }
}
static void smartDistance_main(void)
{
  console_log_info("Enter smartDistance_main()!");

  // 外设的初始化
  PCF8574_Init();   // 初始化IO扩展芯片
  sys_msg_data.pcf8574_io.bit.p1 = 1;   // 关闭蜂鸣器
  PCF8574_Write(sys_msg_data.pcf8574_io.all);
  
  SSD1306_Init(); // OLED 显示屏初始化
  SSD1306_CLS(); // 清屏
  SSD1306_ShowStr(0, 0, " Smart Distance", 16);
  usleep(200 * 1000);

  //  创建线程
  osThreadAttr_t options;
  options.name = "uart_recv_task";
  options.attr_bits = 0;
  options.cb_mem = NULL;
  options.cb_size = 0;
  options.stack_mem = NULL;
  options.stack_size = 1024*5;
  options.priority = osPriorityNormal1;
  uart_recv_task_id = osThreadNew((osThreadFunc_t)uart_recv_task, NULL, &options);
  if (uart_recv_task_id != NULL)
  {
    console_log_info("ID = %d, Create uart_recv_task_id is OK!", uart_recv_task_id);
  }
}

SYS_RUN(smartDistance_main);
