...
if __name__ == '__main__':
        ...
        lastangle = 0
        # 波特率：230400
        # ser= serial.Serial('/dev/ttyUSB0', 230400)  # ubuntu，如果未修改串口别名，可通过 ll /dev 查看雷达具体端口再进行更改
        ser = serial.Serial("COM9", 230400, timeout=5)       # window系统，需要先通过设备管理器确认串口COM号
        while True:
                 ...
                 if data[0] == 0x54 and data[1] == 0x2C:    # 判断是否为数据帧头
                    ...
                    listdata.insert(3, 360-(data[41]*256+data[40])/100.0)   # 原始角度为方便传输放大了100倍，这里要除回去
                    listdata.insert(4, "距离（mm）|光强 *12个点 :")
                    j = 5
                    # for x in range(4, 40, 3):                                 # 2个字节的距离数据，1个信号强度数据，步长为3
                    #     listdata.insert(j, int(data[x+1])*256+int(data[x]))  # 距离
                    #     j += 1
                    #     listdata.insert(j, int(data[x+2]))                   # 信号强度
                    #     j += 1
                    for x in range(37, 1, -3):                                 # 2个字节的距离数据，1个信号强度数据，步长为3
                        listdata.insert(j, int(data[x+1])*256+int(data[x]))  # 距离
                        j += 1
                        listdata.insert(j, int(data[x+2]))                   # 信号强度
                        j += 1
                    ...
                    listdata.insert(30, 360-(data[3]*256+data[2])/100.0)
