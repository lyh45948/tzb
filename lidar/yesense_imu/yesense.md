...
cat > /tmp/fusion.launch << 'EOF'
<launch>
  <!-- IMU filter -->
  <node name="imu_filter" pkg="imu_filter_madgwick"
type="imu_filter_madgwick_node" output="screen">
    <param name="imu_topic" value="/imu"/>
    <param name="publish_tf" value="false"/>
    <param name="fixed_frame" value="odom"/>
  </node>

  <!-- Robot Pose EKF - fuse odometry and IMU -->
...
cat > /tmp/imu_filter.launch << 'EOF'
<launch>
   <node name="imu_filter" pkg="nodelet" type="nodelet" args="standalone 
   imu_filter_madgwick/ImuFilterNodelet" output="screen">
   <param name="imu_topic" value="/imu"/>
   <param name="publish_tf" value="false"/>
   </node>
</launch>
...
# Cartographer SLAM 启动（使用项目自带的 launch 文件）
roslaunch cartographer_ld06 cartographer_ld06.launch use_imu:=true